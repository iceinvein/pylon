...
It connects directly to your existing Claude Code or Codex CLI authentication. No extra accounts. No browser tabs. Just open it and work.

---

## Sessions

Full agent SDK integration with real-time token streaming, extended thinking, and multi-agent orchestration. Switch between Claude (Opus, Sonnet, Haiku) and Codex models from a shared provider registry. Adjust reasoning effort per session. Resume any conversation where you left off — sessions persist in a local SQLite database.
...
Select a pull request from your repo. Pylon dispatches specialized review agents in parallel — security, bugs, performance, style, architecture, UX — each with its own system prompt you can customize. Review agents can run on Claude or Codex models.

Large diffs are automatically chunked for parallel processing, generated dependency files are skipped, and code-intelligence context is attached when available. Findings surface with severity badges, risk/suggestion sections, and precise file/line references. Review them in a split diff view with inline annotations, then post individual findings or a full review directly to GitHub. Previous findings are tracked so active issues can be revalidated across new commits.
...
An AST-powered code explorer for your project. The repo map gives a high-level structural overview. Select any file to visualize its syntax tree, with a code panel and minimap for navigation. The impact explorer traces imports, reverse imports, references, calls, and related tests from a selected symbol. An AI chat panel lets you ask questions about the code structure.
...
- A [Claude Code](https://claude.ai/code) login for Claude models, or a Codex CLI login for Codex models
...
Pylon uses your existing Claude Code and Codex CLI authentication. It does not require raw API keys for normal agent sessions.
...
| Layer | Technology |
|-------|------------|
| Runtime | [Electron 42](https://www.electronjs.org/) + [electron-vite](https://electron-vite.org/) |
| Frontend | [React 19.2](https://react.dev/), [Tailwind CSS 4.3](https://tailwindcss.com/), [Zustand](https://zustand.docs.pmnd.rs/) |
