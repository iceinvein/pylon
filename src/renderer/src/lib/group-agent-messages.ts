...
export function groupAgentMessages(sessionMessages: unknown[]): GroupedMessages {
  ...
  for (let i = 0; i < msgs.length; i++) {
    ...
    if (msg.type === 'user') {
      const content = (msg.message?.content ?? msg.content ?? []) as ContentBlock[]
      for (const block of content) {
        if (block.type === 'tool_result' && block.tool_use_id && agentToolUseIds.has(block.tool_use_id)) {
          const agent = agentMap.get(block.tool_use_id)
          if (agent) {
            agent.done = true
            if ((block as any).is_error) agent.isError = true
            // Extract text result
  ...
  const hasSessionResult = msgs.some((m) => m.type === 'result' && (m as any).total_cost_usd !== undefined)
  if (hasSessionResult) {
