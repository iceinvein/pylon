...
export function PrFilesChanged({ files, diff }: PrFilesChangedProps) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-lg border border-base-border-subtle bg-base-surface/40">
      {/* Summary header */}
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs transition-colors hover:bg-base-raised/50"
      >
        <ChevronDown
          size={12}
          className={`shrink-0 text-base-text-muted transition-transform duration-150 ${expanded ? '' : '-rotate-90'}`}
        />
        <FileText size={12} className="shrink-0 text-base-text-muted" />
        ...
        <span className="ml-auto flex items-center gap-2 font-mono tabular-nums">
          ...
          <span className="text-error">-{formatNumber(totalDeletions)}</span>
        </span>
      </button>
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            <div className="max-h-[70vh] overflow-y-auto border-base-border-subtle border-t">
              {files.map((file, i) => (
                <FileRow
                  key={file.path}
                  file={file}
...
function FileRow({
  ...
  return (
    <div className={isLast && !isExpanded ? '' : 'border-base-border-subtle/50 border-b'}>
      <button
        type="button"
        onClick={hasDiff ? onToggle : undefined}
        className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs transition-colors ${
          hasDiff ? 'cursor-pointer hover:bg-base-raised/40' : 'cursor-default'
        } ${isExpanded ? 'bg-base-raised/30' : ''}`}
      >
        {hasDiff ? (
          isExpanded ? (
            <ChevronDown size={11} className="shrink-0 text-base-text-muted" />
          ) : (
            <ChevronRight size={11} className="shrink-0 text-base-text-muted" />
          )
        ) : (
...
function FileDiffContent({ rawDiff }: { rawDiff: string }) {
  ...
  if (hunks.length === 0) {
    return (
      <div className="border-base-border-subtle/30 border-t bg-base-bg/50 px-3 py-2 text-base-text-faint text-xs">
  ...
  return (
    <div className="border-base-border-subtle/30 border-t bg-base-bg/50">
