...
export function PlanCard({ plan, sessionId, sectionTitles }: PlanCardProps) {
  ...
  return (
    <div
      className={`max-w-[520px] overflow-hidden rounded-lg border ${statusColor} bg-stone-900/80`}
    >
      {/* Header */}
      <div className="flex items-center gap-2.5 border-stone-800 border-b px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate font-semibold text-[13px] text-stone-200">{fileName}</div>
      ...
      {sectionTitles.length > 0 && (
        <div className="flex flex-col gap-1 px-4 py-2.5">
          {sectionTitles.slice(0, 6).map((title, i) => (
            <div key={title} className="flex items-center gap-2 text-stone-400 text-xs">
              <span className="text-stone-600">{i + 1}.</span>
              <span className="truncate">{title}</span>
            </div>
          ))}
          {sectionTitles.length > 6 && (
            <div className="text-stone-600 text-xs">+ {sectionTitles.length - 6} more</div>
          )}
        </div>
      )}
      ...
      {/* Action bar */}
      {plan.status === 'pending' && (
        <div className="flex items-center gap-2 border-stone-800 border-t px-4 py-2.5">
          <button
            type="button"
      ...
      {plan.status === 'changes_requested' && (
        <div className="flex items-center gap-2 border-stone-800 border-t px-4 py-2.5">
          <button
            type="button"
            onClick={handleOpenReview}
            className="flex-1 rounded-md border border-amber-700/50 bg-amber-900/20 px-4 py-2 font-medium text-[13px] text-amber-300 transition-colors hover:bg-amber-900/40"
          >
            Review Again
          </button>
        </div>
