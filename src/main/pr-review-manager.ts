...
class PrReviewManager {
  ...
  private async runAgentSession(
    ...
    try {
      ...
      const byTool = Object.entries(tally.byTool)
        .map(([k, v]) => `${k}=${v}`)
        .join(' ')
      logger.info(
        `Agent ${focus} MCP tool usage for review ${reviewId}: attempts=${tally.attempts} errors=${tally.errors}${byTool ? ` (${byTool})` : ''}`,
      )
    } catch (telemetryErr) {
      logger.warn(`Agent ${focus} failed to tally MCP tool usage: ${String(telemetryErr)}`)
    }
